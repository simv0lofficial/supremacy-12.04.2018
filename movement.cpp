...
void Movement::QuickStop( ) {
	...
	if( speed > 20.f ) {
		g_cl.m_cmd->m_forward_move = direction.x * -450.f;
		g_cl.m_cmd->m_side_move = direction.y * -450.f;
	}
	else {
		g_cl.m_cmd->m_forward_move = 0.f;
...
void Movement::FakeWalk( ) {
	...
	if( ticks > ( ( max - 2 ) - g_csgo.m_cl->m_choked_commands ) || !g_csgo.m_cl->m_choked_commands || g_csgo.m_cl->m_choked_commands >= g_cl.m_max_lag - 2 ) {
		Movement::QuickStop( );
	}
}