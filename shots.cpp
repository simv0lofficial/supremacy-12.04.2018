...
void Shots::OnHurt( IGameEvent *evt ) {
	...
	if ( group == HITGROUP_HEAD || group == HITGROUP_RIGHTLEG || group == HITGROUP_LEFTLEG ) {
		...
		case Resolver::Modes::RESOLVE_STAND:
			...
			data->m_prefer_stand.push_front( record->m_eye_angles.y );
			break;

		case Resolver::Modes::RESOLVE_AIR:
			data->m_prefer_air.clear( );
			data->m_prefer_air.push_front( record->m_eye_angles.y );
			break;
