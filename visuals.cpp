...
void Visuals::StatusIndicators( ) {
	...
	for( size_t i{ }; i < indicators.size( ); ++i ) {
		...
		render::indicator.string( 20, g_cl.m_height - 110 - ( 30 * i ), indicator.color, indicator.text );
	}
}
	...
	// draw bottom bars.
	{
		...
		if( enemy && g_menu.main.players.lby_update.get( ) ) {
			...
			if( data && data->m_records.size( ) ) {
				...
				if( current ) {
					if( !( current->m_velocity.length_2d( ) > 0.1 && !current->m_fake_walk ) && data->m_body_update != FLT_MAX && data->m_body_index <= 3 ) {
						// calculate box width
						float cycle = std::clamp<float>( data->m_body_update - current->m_anim_time, 0.f, 1.1f );
						float width = ( ( box.w - 2 ) * ( cycle / 1.1f ) );
