...
void AimPlayer::UpdateAnimations( LagRecord *record ) {
	...
	if ( record->m_lag > 1 && !bot ) {
		...
		if ( ( record->m_flags & FL_ONGROUND )
			&& ( record->m_layers[ 6 ].m_weight == 0.f || record->m_layers[ 6 ].m_playback_rate == 0.f )
			&& speed > 0.1f && speed < 125.f )
		...
		if ( m_records.size( ) >= 2 ) {
			...
			if ( previous && !previous->dormant( ) ) {
				...
				// set ground flag to previous sent.
	...
	for( auto& cur : m_records ) {

		if( size > 5 )
			break;

		consistency += cur->m_lag;
		++size;
	}

	float scale = static_cast< float >( consistency ) / static_cast< float >( size ); 
	bool fake = scale > 1.f && g_menu.main.aimbot.correct.get( );
	...
	// update animations.
	g_hooks.m_UpdateClientSideAnimation( m_player );
...
void AimPlayer::SetupHitboxes( LagRecord *record, bool history ) {
	...
	// standing.
	if( record->m_velocity.length_2d( ) == 0.f ) {
		size_t iter = 0;

		while( m_prefer_stand.size( ) > 2 )
			m_prefer_stand.pop_back( );

		// get all prefered angles.
		for( auto& yaw : m_prefer_stand ) {

			// previously hit head angle.
			if( std::abs( math::NormalizedAngle( yaw - record->m_eye_angles.y ) ) <= 17.5f )
				++iter;
		}
	...
	// in air.
		...
		while( m_prefer_air.size( ) > 2 )
			m_prefer_air.pop_back( );

		// get all prefered angles.
		for( auto& yaw : m_prefer_air ) {
...
bool AimPlayer::GetBestAimPosition( vec3_t &aim, float &damage, LagRecord *record ) {
	...
	for ( const auto &it : m_hitboxes ) {
		...
		for ( const auto &point : points ) {
			...
			// ignore mindmg.
			if ( it.m_mode == HitscanMode::LETHAL )
				in.m_damage = in.m_damage_pen = hp;
			else if( it.m_mode == HitscanMode::LETHAL2 )
				in.m_damage = in.m_damage_pen = hp * 0.5f;
			...
			if ( penetration::run( &in, &out ) ) {
				...
				if ( it.m_mode == HitscanMode::PREFER && out.m_damage >= hp * 0.5f )
					done = true;

				// this hitbox requires lethality to get selected, if that is the case.
				// we are done, stop now.
				else if ( it.m_mode == HitscanMode::LETHAL )
					done = true;
...
void Aimbot::apply( ) {
	...
	if ( attack || attack2 ) {
		// choke every shot.
		*g_cl.m_packet = g_cl.m_lag >= g_cl.m_max_lag;
		...
		if ( m_target ) {
			...
			// store fired shot.
			g_shots.OnShotFire( m_target, m_damage, 0, m_record );
		}
