...
public:
	...
	void SetInterpolation( bool interp = false )
	{
		...
		{
			auto e = &varmapping->m_Entries.Element( j );
...
public:
	...
	__forceinline vec3_t CalculateSpread( int seed, float inaccuracy, float spread ) {
		WeaponInfo *wep_info;
		int        item_def_index;
		...
		if ( wep_info->m_weapon_type == WEAPONTYPE_SHOTGUN && g_csgo.weapon_accuracy_shotgun_spread_patterns->GetInt( ) > 0 )
			g_csgo.GetShotgunSpread( item_def_index, 0, wep_info->m_bullets * recoil_index, &r4, &r3 );
