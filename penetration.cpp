...
bool penetration::TraceToExit( const vec3_t &start, const vec3_t& dir, vec3_t& out, CGameTrace* enter_trace, CGameTrace* exit_trace ) {
	...
	while( dist <= 90.f ) {
		...
		// set out pos.
		out = start + ( dir * dist );
		contents = g_csgo.m_engine_trace->GetPointContents( out, CS_MASK_SHOOT | CONTENTS_HITBOX );
...
bool penetration::run( PenetrationInput_t* in, PenetrationOutput_t* out ) {
	...
	zeus = weapon->m_iItemDefinitionIndex( ) == ZEUS;

	// check if it's an actual gun.
	if( !zeus && ( weapon_info->m_weapon_type > WEAPONTYPE_MACHINEGUN || weapon_info->m_weapon_type < WEAPONTYPE_PISTOL ) )
		return false;
    ...
    while( damage > 0.f ) {
		...
		damage_lost = ( ( modifier * 3.f ) * penetration_mod + ( damage * damage_mod ) ) + ( ( ( exit_trace.m_endpos - trace.m_endpos ).length_sqr( ) * modifier ) / 24.f );
